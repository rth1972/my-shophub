...
const ProductItem = ({ item }:Product) => {
  ...
  const productName = item.product_name || item.title || 'Untitled Product';
  const productPrice = item.price || 0;
  const discountPrice = item.discountedPrice || item.price || 0;
  const reviews = item.reviews || 0;
  const productId = item.product_id || item.id || 1;
  const categoryName = item.category_name || '';
  const stockQuantity = item.stock_quantity ?? null;

  // ✅ CRITICAL LINE: setModalContent is now available from the corrected context
  ...
  const handleQuickView = (e: React.MouseEvent<HTMLElement>) => {
    ...
    // Set the product data in the context state
    setModalContent(item); 
    // Open the modal
    openModal();         
  };

  // Simple click handler for the whole card area (optional, removed from div for simplicity)
  const handleProductClick = () => {
    // Removed to prevent conflict with link/button clicks
  };

  return (
    <div className="group  overflow-hidden"> 
      <div className="relative overflow-hidden flex items-center justify-center rounded-t-lg h-64 mb-4 border border-gray-2">
        <Image 
          src={imageUrl} 
          alt={productName} 
          fill
        ...
        <div className="absolute top-3 left-3 bg-red text-white px-3 py-1 rounded-full text-xs font-medium flex items-center gap-1 z-10">
        ...
        {stockQuantity !== null && stockQuantity < 10 && stockQuantity > 0 && (
        ...
        {stockQuantity === 0 && (
          <div className="absolute top-3 right-3 bg-red text-white px-3 py-1 rounded-full text-xs font-medium z-10">
            Sold Out
          </div>
        )}
        ...
        {/* <div className="absolute left-0 bottom-0 translate-y-full w-full flex items-center justify-center gap-2.5 pb-5 ease-linear duration-200 group-hover:translate-y-0 z-10">
          <button
            ...
            className="flex items-center justify-center w-9 h-9 rounded-[5px] shadow-1 ease-out duration-200 text-dark bg-white hover:text-pink-600"
          >
            <svg
          ...
          <button  onClick={() => onAddToCart(item)}
            className="inline-flex font-medium text-custom-sm py-[7px] px-5 rounded-[5px] bg-blue text-white ease-out duration-200 hover:bg-blue-dark shadow-md disabled:bg-gray-400 disabled:cursor-not-allowed"
            disabled={stockQuantity === 0}
            // Add cart logic here
          >
            {stockQuantity === 0 ? 'Sold Out' : 'Add to cart'}
          </button>
      ...
      {categoryName && (
        <div className="mb-2">
          <span className="inline-block text-xs font-medium text-pink-600 bg-pink-50 px-2 py-1 rounded">
            {categoryName}
          </span>
        </div>
      )}
      ...
      <div className="flex items-center gap-2.5 mb-2 px-2 py-1">
        ...
        <p className="text-custom-sm text-gray-600">({reviews})</p>
      </div>
<div className="px-2 py-1">
      {/* ✅ Open Modal on Title Click (prevent link default behavior) */}
      <h3 
        className="font-medium text-dark ease-out duration-200 hover:text-pink-600 mb-1.5"
      >
        <Link 
          href={`/shop-details/${productId}`} 
          onClick={handleQuickView} // This now calls the handler and stops link navigation
        > 
          {productName} 
        </Link>
      </h3>
      ...
      <div className="flex items-center gap-2">
        <span className="font-bold text-lg text-pink-600">${productPrice}</span>
        {discountPrice !== productPrice && discountPrice > 0 && (
          <span className="text-gray-400 line-through text-sm">${discountPrice}</span>
        )}
      </div>
<div className="relative mt-3 translate-y-0 w-full flex items-center justify-center gap-2.5 pb-5 ease-linear duration-200 group-hover:translate-y-0 z-10">
          {/* ✅ QUICK VIEW BUTTON: Call the local handler */}
          <button
            onClick={handleQuickView}
            id="newOne"
            aria-label="button for quick view"
            className="flex items-center justify-center w-9 h-9 rounded-[5px] ease-out duration-200 text-dark bg-gray-1 hover:text-pink-600"
          >
            {/* SVG content for quick view icon */}
            <svg
              className="fill-current"
              width="16"
              height="16"
          ...
          {/* Add to Cart button (no change necessary) */}
      ...
      <p className="text-xs text-gray-500 mt-1 hidden">Gently used condition</p>
      </div>
    </div>
  );
};
