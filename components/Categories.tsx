...
const Categories = () => {
  ...
  useEffect(() => {
    // This effect might not be needed if Swiper initializes correctly on its own
    // but we'll keep the initialization logic for safety.
    // In many setups, removing this is fine.
    ...
    <section className="overflow-hidden pt-17.5">
      <div className="max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0 pb-15 border-b border-gray-3">
        <div className="swiper categories-carousel common-carousel">
          ...
          >
            {/* 4. Use the 'categories' prop instead of local 'data' */}
            {categories.map((item, key) => (
              <SwiperSlide key={key}>
                {/* You can pass the selectedCategory down to SingleItem if needed for styling */}
                <SingleItem 
                  item={item} 
                />
              </SwiperSlide>
